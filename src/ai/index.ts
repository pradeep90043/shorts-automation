...
`;
    ...
    if (provider === "mock") {
      pipelineLogger.info(
        "AI Provider is set to mock, using rule-based fallback generator.",
        "AIService",
      );
    ...
    if (provider === "freellmapi") {
      pipelineLogger.info(
        "Using FreeLLMAPI to generate metadata...",
        "AIService",
      );
      if (!config.ai.freellmapiKey) {
        pipelineLogger.warn(
          "FreeLLMAPI API key is missing. Falling back to rule-based generation.",
          "AIService",
        );
        return this.generateFallbackMetadata(cleanOcr);
      }
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            "FreeLLMAPI returned JSON, but structure is missing required fields.",
          );
        }

        pipelineLogger.checkpoint(
          "FreeLLMAPI metadata generated",
          true,
          `Title: "${parsed.title}"`,
        );
        return parsed;
        ...
        pipelineLogger.warn(
          `FreeLLMAPI API metadata generation failed: ${err instanceof Error ? err.message : err}. Falling back to rule-based generation.`,
          "AIService",
        );
        return this.generateFallbackMetadata(cleanOcr);
      }
    ...
    try {
        ...
        exec(cmd, (error, stdout, stderr) => {
          if (error) {
            pipelineLogger.error(
              `AI CLI command execution failed: ${stderr}`,
              error,
              "AIService",
            );
            reject(error);
      ...
      if (!parsed.title || !parsed.description || !Array.isArray(parsed.tags)) {
        throw new Error(
          "AI returned JSON, but structure is missing required fields.",
        );
      }
      ...
      pipelineLogger.warn(
        `AI CLI processing failed: ${err instanceof Error ? err.message : err}. Falling back to rule-based generation.`,
        "AIService",
      );
      return this.generateFallbackMetadata(cleanOcr);
    } finally {
  ...
  private generateFallbackMetadata(ocrText: string): VideoMetadata {
    ...
    if (
      ...
      text.includes("def ") ||
      (text.includes("import ") && text.includes(".py"))
    ) {
      topic = "Python";
      ...
      (text.includes("image:") && text.includes("container"))
    ) {
      topic = "Docker";
