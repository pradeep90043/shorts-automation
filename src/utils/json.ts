...
export function parseAiJson<T>(raw: string): T {
  ...
  try {
    ...
    pipelineLogger.warn(`Initial JSON parse failed. Attempting repair... Error: ${parseErr instanceof Error ? parseErr.message : parseErr}`, 'JsonParser');
  }
  ...
  try {
    ...
    pipelineLogger.error(`JSON repair failed. Content attempted to parse:\n${s}`, repairErr as Error, 'JsonParser');
    throw new Error(`JSON parsing failed even after repair: ${repairErr instanceof Error ? repairErr.message : repairErr}`);
  }
}
