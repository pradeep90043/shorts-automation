...
export class MusicService implements IMusicService {
  public async addBackgroundMusic(
    ...
    pipelineLogger.info(`Adding background music to video ${videoPath} (Mood: ${mood || 'unspecified'})`, 'MusicService');
    ...
    try {
      ...
      for (const folder of foldersToTry) {
        if (fs.existsSync(folder)) {
          ...
          if (files.length > 0) {
            audioTracks = files;
            pipelineLogger.info(`Found ${files.length} tracks in folder: ${folder}`, 'MusicService');
            break;
          }
        }
      }
