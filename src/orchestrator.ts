...
export class PipelineOrchestrator {
  ...
  public async resumePipeline(context: PipelineContext): Promise<void> {
    pipelineLogger.info(`Resuming pipeline ${context.id} from manual review override.`, 'Orchestrator');
  ...
  public async run(context: PipelineContext): Promise<void> {
    ...
    try {
      ...
      pipelineLogger.checkpoint('Image analyzed', true, `${context.analysis.width}x${context.analysis.height}`);
      ...
      if (context.instagramSource) {
        pipelineLogger.info('Instagram source detected — skipping branding detection & removal', 'Orchestrator');
        context.cleanedImagePath = context.originalImagePath;
        await this.runFromStage(context, 'ocr');
        return;
      }
      ...
      context.brandingDetection = await this.detector.detect(context.originalImagePath, context.analysis);
      pipelineLogger.checkpoint('Branding detection completed', true, `Zones: ${context.brandingDetection.zones.length}`);
      ...
      if (!removalResult.success) {
        ...
        pipelineLogger.checkpoint('Branding removal bypassed', false, removalResult.preservationReason);
   ...
   */
  private async runFromStage(context: PipelineContext, startStage: 'ocr' | 'generating_layout'): Promise<void> {
    const chatId = context.telegramMeta.chatId;
    const messageId = context.telegramMeta.messageId;
    ...
    try {
      ...
      if (useAI) {
        ...
        context.brandedImagePath = await this.branding.applyCodeOrCapBranding(
          context.layoutImagePath, brandedPath
        );
      } else {
        ...
        context.layoutImagePath = await this.layout.generate(
          workingImage, context.analysis!, layoutPath, context.ocr?.text
        );
      ...
      context.metadata = await this.ai.generateMetadata(context.ocr?.text || '', imageCtx);
      ...
      await this.telegramService.sendMessage(chatId, `🚀 *Rendering completed!* Uploading to YouTube Shorts...`, messageId);
      
      const uploadResult = await this.youtube.uploadShort(context.finalVideoPath, context.metadata);
      context.youtubeUrl = uploadResult.url;
      context.youtubeVideoId = uploadResult.videoId;

      // 11. Completed
      context.status = 'completed';
      pipelineLogger.checkpoint('Short generated and uploaded successfully', true, `Gen ID: ${context.id}`);
  ...
  private saveContextState(context: PipelineContext): void {
    try {
      ...
      pipelineLogger.info(`Saved context state to ${statePath}`, 'Orchestrator');
    } catch (err) {
  ...
  public getSavedContext(id: string): PipelineContext | null {
    try {
      ...
      pipelineLogger.error(`Failed to retrieve saved context ${id}`, err, 'Orchestrator');
    }
    return null;
  }
  ...
  private cleanupTempFiles(context: PipelineContext): void {
    try {
      ...
      if (context.renderedVideoPath && fs.existsSync(context.renderedVideoPath)) {
      ...
      pipelineLogger.info(`Cleaned up temporary video files for ${context.id}`, 'Orchestrator');
    } catch (err) {
      pipelineLogger.warn(`Failed to clean up temp files: ${err instanceof Error ? err.message : err}`, 'Orchestrator');
    }
  }

  private async handlePipelineFailure(context: PipelineContext, error: any): Promise<void> {
