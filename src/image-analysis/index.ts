...
export class ImageAnalyzer implements IImageAnalyzer {
  public async analyze(imagePath: string): Promise<ImageAnalysisResult> {
    ...
    try {
      ...
      const { hasBlackMargins, hasWhiteMargins } =
        await this.detectMargins(image);
      ...
      pipelineLogger.error(
        `Failed to analyze image at ${imagePath}`,
        err,
        "ImageAnalysis",
      );
      throw err;
  ...
  private async detectMargins(
    ...
    try {
      ...
      pipelineLogger.warn(
        `Margin detection failed: ${err instanceof Error ? err.message : err}. Defaulting to false.`,
        "ImageAnalysis",
      );
