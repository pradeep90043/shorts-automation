  ...
  public async extractText(imagePath: string): Promise<OcrResult> {
    ...
    try {
      ...
      const result = await Tesseract.recognize(imagePath, "eng", {
        logger: (m) => {
          if (
            m.status === "recognizing" &&
            Math.round(m.progress * 100) % 25 === 0
          ) {
            pipelineLogger.info(
      ...
      const words = result.data.words
        ? result.data.words.map((w) => w.text)
        : [];
      const detailedWords = result.data.words
      ...
      pipelineLogger.checkpoint(
        "OCR completed",
        true,
        `Extracted ${words.length} words with confidence ${confidence}%`,
      );
