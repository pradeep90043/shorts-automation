...
export class BrandingRemover implements IBrandingRemover {
  public async remove(
    ...
    pipelineLogger.info(
      `Initiating branding removal on image: ${imagePath}`,
      "BrandingRemover",
    );

    if (zones.length === 0) {
      pipelineLogger.info(
        "No branding zones detected. Skipping removal.",
        "BrandingRemover",
      );
      return {
        success: true,
    ...
    try {
      ...
      const edgeTopZones = zones.filter(
        (z) => z.boundingBox.y === 0 && z.boundingBox.height <= 0.12,
      );
      const edgeBottomZones = zones.filter(
        (z) => z.boundingBox.y >= 0.88 && z.boundingBox.height <= 0.12,
      );
      ...
      if (middleZones.length > 0) {
        ...
        return {
          ...
          outputPath: imagePath,
          preservationReason:
            "Embedded middle watermark detected, flagged for manual review",
        };
      }
      ...
      if (totalCropPercentage > 0.2) {
        ...
        return {
          ...
          outputPath: imagePath,
          preservationReason:
            "Calculated crop ratio is too high, potential content loss",
        };
      }
      ...
      if (cropTop > 0 || cropBottom > 0) {
        ...
        pipelineLogger.checkpoint(
          "Branding removed",
          true,
          `Cropped via edge-shaving. Output saved to ${croppedPath}`,
        );
